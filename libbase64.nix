    ...
    let
      ...
      flag =
        ...
        in if builtins.isNull s
           then builtins.throw
                  ("aklomp: invalid SIMD extension: \"${ext}\".\nValid SIMD extensions are: ${builtins.toString validExts}")
           else s;
    in if flag.isSupported then flag else builtins.throw "aklomp: ${ext} is not supported on your platform.";
...
*/
  ...
  supportToFlag = { ext, isSupported, flagRhs, ... }@support:
    let suffix = if ext == "OPENMP" then "" else "_CFLAGS";
        ...
        withWarnings = x: if warnings != [] then builtins.trace (builtins.concatStringsSep "\n" warnings) x else x;
        enableDebug = flag: x: builtins.trace "enabling: ${flag}" x;
        disableDebug = flag: x: builtins.trace "not enabling: ${flag}" x;
    in if isSupported
